  ...
  return new Promise(async (resolve, reject) => {
    try {
      const [restorable_type_id] = await knex('events.types')
        .where({ label: 'Pinta-remontti' })
        .pluck('id');

      if (!restorable_type_id) {
        //Ignore everything if no targets exist.
        resolve();
      }
      ...
      for await (const entry of stream) {
        ...
        await trx('events.renovateable_target_type').insert({
          target_id: targetId,
        });
      }
      await trx.commit();
...
exports.down = function (knex) {
  return new Promise(async (resolve, reject) => {
    try {
      const trx = await knex.transaction();
      const stream = await trx('events.renovatable_target_type');
