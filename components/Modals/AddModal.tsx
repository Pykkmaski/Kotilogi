...
import { useAddModal } from './BaseAddModal.hooks';
...
export function AddPropertyModal({ refId, ...props }: AddModalProps) {
  ...
  return (
    <BaseAddModal {...props} submitMethod={onSubmit} title='Lisää Talo'>
      ...
      <Input name='title' label='Osoite' description='Talon katuosoite.' placeholder='Kirjoita osoite...' required={true} autoComplete='off' onChange={updateData} />
      ...
      <Textarea label='Kuvaus' description='Talon lyhyt kuvaus.' placeholder='Kirjoita kuvaus...' spellCheck={false} name='description' onChange={updateData} />

      <div className='flex flex-col w-full'>
        <div className='flex items-center w-full gap-4 justify-end'>
          <span className='text-slate-500'>
            Kohteen avausmaksu on <span className='text-green-600 text-lg'>(9,90€)</span>
          </span>
        </div>
        <small className='text-sm text-slate-500 text-right mt-4'>
          Avausmaksua ei tarvitse maksaa heti.
          <br />
...
export function AddEventModal({ refId, ...props }: AddModalProps) {
  ...
  return (
    <BaseAddModal {...props} submitMethod={onSubmit} title='Lisää Tapahtuma'>
      <Input name='title' label='Otsikko' description='Tapahtuman otsikko.' placeholder='Kirjoita otsikko...' required={true} autoComplete='off' onChange={updateData} />

      <Input name='time' label='Päiväys' description='Tapahtuman päivämäärä.' required type='date' onChange={updateData} />

      <Textarea label='Kuvaus' description='Tapahtuman lyhyt kuvaus.' placeholder='Kirjoita kuvaus...' spellCheck={false} name='description' onChange={updateData} />
...
export function AddFilesModal({ accept, tablename, refId, uploadMethod, ...props }: AddFilesModalProps) {
  ...
  return (
    <BaseAddModal {...props} submitMethod={onSubmit} title='Lisää Tiedostoja'>
      <Input name='file' type='file' accept={accept} onInput={updateFiles} required={true} label='Tiedostot' description='Valitse tiedostot.' />
    </BaseAddModal>
  );
}
