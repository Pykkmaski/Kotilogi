...
export type ActionType<T extends Kotilogi.ItemType> = {
    type: 'select_item',
    value: T,
} | {
    type: 'reset_selected',
...
export function reducer<T extends Kotilogi.ItemType>(state: StateType<T>, action: ActionType<T>): StateType<T>{
    switch(action.type){
        case 'select_item':{
