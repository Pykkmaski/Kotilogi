...
export function PropertyImageListItem(props: ListItemProps<Kotidok.FileType>) {
  return <ImageListItem {...props} tablename='propertyFiles' deleteMethod={() => deletePropertyFile(props.item.id)} />;
}

export function EventImageListItem(props: ListItemProps<Kotidok.FileType>) {
  return <ImageListItem {...props} tablename='eventFiles' deleteMethod={() => deleteEventFile(props.item.id)} />;
}
