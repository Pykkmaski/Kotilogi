...
ListItem.Header = function({children}){
    return (
        <div className="w-full items-center flex justify-between text-black">
            {children}
        </div>
    )
}
...
ListItem.Footer = function({children}){
    return (
        <div className="w-full flex items-center text-slate-400 mt-4">
            {children}
        </div>
    )
}
...
ListItem.Controls = function({children}){
    return (
        <div className="flex h-full justify-self-end">{children}</div>
    );
}
...
ListItem.CheckBox = function({checked}: CheckBoxProps){
    ...
    return (
        <input className="aspect-square xs:w-[1.5rem] lg:w-4" type="checkbox" onChange={() => dispatch({
            type: 'select_item',
