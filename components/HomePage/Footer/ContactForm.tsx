...
function ContactForm(props) {
  ...
  const FormGroup = ({ children }) => <div className='flex flex-col gap-2 text-slate-500'>{children}</div>;
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col gap-4 md:w-[600px] xs:w-full' ref={formRef}>
      ...
      <div className='w-full'>
        <PrimaryButton type='submit' id='contact-submit-button' className='w-full text-black text-center justify-center font-semibold'>
          Lähetä
        </PrimaryButton>
      </div>
