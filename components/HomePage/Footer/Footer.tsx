...
const LinkIcon: React.FC<LinkIconProps> = ({ children, variant, ...props }) => {
  const className = ['fa text-white cursor-pointer hover:no-underline before:text-[42px]', variant === 'ig' ? 'fa-instagram' : variant === 'sc' ? 'fa-soundcloud' : ''];

  return (
    <a className={className.join(' ')} target='_blank' {...props}>
      {children}
    </a>
  );
};
