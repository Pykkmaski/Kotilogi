...
function Controls(props: ControlsProps) {
  ...
  return (
    <div className={className.join(' ')} hidden={props.hidden}>
      {props.editing ? (
        <Group direction='row' gap={4}>
          <SecondaryButton data-testid='cancel-btn' onClick={props.cancelEdit} hidden={loading}>
            Peruuta
          </SecondaryButton>

          <PrimaryButton type='button' onClick={props.onSubmit} loading={loading} disabled={loading}>
            Tallenna
          </PrimaryButton>
        </Group>
      ) : (
        <PrimaryButton data-testid='edit-btn' type='button' onClick={() => props.edit()}>
          Muokkaa
        </PrimaryButton>
      )}
    </div>
...
export function SingleInputForm({ inputComponent: InputComponent, editingDisabled = false, ...props }: SingleInputFormProps) {
  const { editing, status, edit, cancelEdit, onSubmit, cancelFallbackValue, inputValue } = useSingleInputForm(props.initialInputProps);
...
export function SingleSelectForm({ inputComponent: InputComponent, childComponent: ChildComponent, ...props }: SingleSelectFormProps) {
  const { editing, inputValue, edit, cancelEdit, status, onSubmit, cancelFallbackValue } = useSingleInputForm<SelectProps>(props.initialInputProps);
        ...
        <InputComponent
          ...
          {editing
            ? props.childProps.map((childProps, index: number) => {
                ...
                return (
                  <ChildComponent {...childProps} selected={selected} key={`input-option-${inputId}-${index}`}>
                    {' '}
                    {childProps.children}{' '}
                  </ChildComponent>
                );
