      ...
      <div className="flex flex-col gap-4 items-center">
        <div className="flex w-full justify-between gap-4 xs:items-start md:items-center">
          <img
            ...
            className="aspect-square xs:w-[30px] md:w-[80px] xs:mt-4 md:mt-0"
          ></img>
          <span className="text-lg text-slate-500">
            Käytämme evästeitä. Kaikki evästeet ovat sovelluksen toiminnan
            kannalta välttämättömiä, eikä niitä käytetä yksityisten tietojen
            keräämiseen.
            <br />
            Jatkamalla sivuston käyttöä, hyväksyt evästeiden tallennuksen
            laitteellesi.
          </span>
        </div>
...
export function CookieNotice() {
  ...
  useEffect(() => {
    setCookiesAccepted(
      () => localStorage.getItem("kotidok-cookies") === "true"
    );
  }, []);
  ...
  return (
    <div className="fixed bottom-4 xs:right-0 md:right-4 shadow-lg xs:w-full md:w-[500px] z-50 animate-slideup-slow flex items-center">
      {!cookiesAccepted ? (
        <Message setCookiesAccepted={setCookiesAccepted} />
      ) : null}
    </div>
  );
}
