...
const Gradient = () => <div className='absolute top-0 left-0 w-full h-full bg-gradient-to-r from-primary from-50% to-transparent z-10' />;

function HeroBgImage() {
  return <div className='absolute top-0 left-0 w-full h-full bg-houses bg-center bg-cover grayscale xs:hidden lg:block' />;
}

function HeroSection() {
  return (
    <div className='flex flex-row items-center w-full relative py-32 min-h-[500px] bg-primary'>
      <Gradient />
      <HeroBgImage />
      <div className='z-10'>
        <Padding>
...
function Customers() {
  ...
  const CustomerProfile = ({ imageUrl, name, flipped, text }) => {
    const className = ['flex gap-8 w-full xs:items-center md:items-start xs:flex-col', flipped ? 'md:flex-row-reverse' : 'md:flex-row'];
    ...
    <section className='flex flex-col md:px-64 xs:px-4 py-20 bg-primary'>
      ...
      <div className='flex flex-col gap-32'>
        {customerItems.map((item, index) => {
          return <CustomerProfile imageUrl={'/blank_profile.png'} name={item.name} text={item.text} flipped={index % 2 == 0} />;
        })}
      </div>
    </section>
  );
