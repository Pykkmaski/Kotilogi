  ...
  return (
    <SubmitDataModal
      ...
      submitMethod={async (data, files?) => {
        await addEvent({ ...data, refId: propertyId } as TODO, files).then(() => toast.success('Tapahtuman lisäys onnistui!'));
      }}>
      <Modal.Header>
      ...
      <Modal.Body>
        <SubmitDataModal.Form className='flex flex-col gap-4 lg:min-w-[700px] xs:w-full'>
          <Input name='title' label='Otsikko' description='Tapahtuman otsikko.' placeholder='Kirjoita otsikko...' required={true} autoComplete='off' />

          <Input name='time' label='Päiväys' description='Tapahtuman päivämäärä.' required type='date' />

          <Textarea label='Kuvaus' description='Tapahtuman lyhyt kuvaus.' placeholder='Kirjoita kuvaus...' spellCheck={false} name='description' />
